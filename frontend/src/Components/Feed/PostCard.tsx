...
export default function PostCard({
  ...
  const toggleCommentLike = async (commentId: number) => {
    ...
    const findComment = (commentsArr: CommentResponseDto[], idToFind: number): CommentResponseDto | null => {
      for (const comment of commentsArr) {
        if (comment.commentId === idToFind) return comment
        if (comment.replies) {
          const found = findComment(comment.replies, idToFind)
